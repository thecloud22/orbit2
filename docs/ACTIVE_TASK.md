...
| `check` is not in the authoring vocabulary | A `branch` on whether a value was there is authored; a `check` against a condition — an amount over a threshold, a status equal to something — is not. The model may still propose only `enter`, `activate`, `read`, `done`. |
