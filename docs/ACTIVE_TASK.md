...
| Waiting on a person | `waitingForAPerson` is a status the schema allows and nothing produces. A `handOff` step now ends a run as `handedToAPerson` (2026-09-22), a success carrying the request and the values shown; a run that *pauses* for a person and continues is still not built. |
