...
| 2026-09-23 | **A rule compares with the word the page shows.** Scenario 5's table compared First-time buyer with "first-time buyer" on a page that shows Yes or No, so no first-time buyer was ever referred. Connecting a table now also asks how the page writes each word a condition compares with; Orbit takes the page's word only where the page bears it out (the example's value, or the other answer of a yes/no field), records that as an assumption, and otherwise keeps the procedure's word and asks. | A defect fix, not a design change: the tables were always meant to compare values read from the application. No contract or step vocabulary changed. |
| 2026-09-23 | **A second ending may be separated by the one value the rules said may be absent.** The endings answer described "no such file" without naming the value whose absence means it; the confirmed rules had marked exactly one read as possibly missing, so Orbit uses that one. With two or more, it still refuses and asks. | Scenario 5's missing file halted instead of concluding, on a draft that already said which value it would be. |
| 2026-09-23 | **A press whose page has not moved yet gets ten more seconds before it counts as stuck.** Scenario 8's walk pressed Sign in while the machine was loaded (a browser test suite ran alongside), the navigation took over the two-second ceiling, the next turn was shown /login "unchanged", and the model said the procedure was finished. | The environment triggered it; the walk giving up on a slow page is the defect. The two-second ceiling stays for the common case. |
