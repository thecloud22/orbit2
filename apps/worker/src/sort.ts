...
export const SORT = [
  ...
  '',
  'A sentence that qualifies one particular rule belongs to it: one saying that rule\'s outcome is',
  'normal, is not an error, or happens often. Label it rule. A sentence saying when the whole',
  'procedure is used, or what it covers, is background.',
  'A condition that forbids something ("if it is closed, do not reopen it") decides what happens,',
  'so it is a rule, not wontDo. wontDo is for what must never be done at all.',
  'A condition on something done in the application, or on how the procedure ends ("say so", "tell the',
  'caller the status", "refer the file"), is a rule. A condition on work only a person does outside the',
  'application (phoning, emailing, asking someone) is forAPerson: "phone the loan officer if two',
  'conditions were attached" is forAPerson.',
  'Something that should already be true before the procedure starts ("before you start, make sure the',
  'file has been through automated underwriting") is forAPerson, unless the procedure says where in the',
  'application to look. Orbit does not go and make it true. What to do when it is not true ("if it has',
  'not, send it back to processing") is forAPerson with it.',
  'A sentence that only describes what the application shows or does by itself ("it will show as',
  'approved"), or that points to another procedure ("declines go through the credit review"), decides',
  'nothing here: it is background.',
  '',
