...
const INSTRUCTION = [
  ...
  '',
  'Rules that matter:',
  '- If the procedure names a page — "the decline page", "the pipeline in its no-match state",',
  '  "the session-expiry page" — and the page you are shown is not that one, answer act=open',
  '  with that name. Do NOT do the nearest similar thing on the page you are on. Approving a',
  '  file on the ordinary screen is not approving it on the one the procedure named.',
...
export async function authorFromProcedure(opts: {
  ...
  if (!steps.some((s) => s.kind === 'end')) {
    ...
    if (isGuarded) {
      ...
      // Kept inside the 120 characters a conclusion's label allows, because
      // this summary is what the confirmation screen offers as its default.
      // Three conditions spelled out in full overran it, and the attestation
      // was refused with "endings.0.label — Too big".
      const said_ = buildable.map((c) => `${c.of.label} ${readable(c.is)} ${c.than}`);
      const whenAll = said_.length <= 2 ? said_.join(' and ')
        : `${said_[0]} and ${said_.length - 1} more`;
      const passEnd: Step = { id: crypto.randomUUID(), kind: 'end',
        summary: `${said?.whenFound.label || 'Finish — this conclusion has no name yet'} — when ${whenAll}`.slice(0, 120),
      ...
      const failEnd: Step = { id: crypto.randomUUID(), kind: 'end',
        summary: `${said?.whenAbsent?.label || 'Finish — this conclusion has no name yet'} — otherwise`.slice(0, 120),
